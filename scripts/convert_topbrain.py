...
def read_labelmap(path: Path, fullnames_path: Path | None = None) -> dict[int, dict]:
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), 1):
        ...
        if label < 0 or label in labels or any(c < 0 or c > 255 for c in [red, green, blue]):
        ...
        labels[label] = {
            "shortName": fields[7], "name": fullnames.get(fields[7], fields[7]),
...
def voxel_to_gltf_transform(affine: np.ndarray, source_unit: str) -> np.ndarray:
    ...
    if not np.allclose(affine[3], [0, 0, 0, 1]) or abs(np.linalg.det(affine[:3, :3])) < 1e-12:
...
def label_mesh(volume: np.ndarray, label: int, transform: np.ndarray) -> tuple[trimesh.Trimesh, dict]:
    """Extract a complete 0.5 isosurface after bounding-box crop and zero padding."""
    mask = volume == label
    occupied = [np.flatnonzero(np.any(mask, axis=tuple(a for a in range(3) if a != axis))) for axis in range(3)]
    ...
    vertices, faces, _, _ = marching_cubes(padded, level=0.5, gradient_direction="ascent", allow_degenerate=False)
    ...
    return mesh, {
        ...
        "vertexCount": len(mesh.vertices), "triangleCount": len(mesh.faces),
        "boundsGltfM": bounds, "closedSurface": bool(mesh.is_watertight),
        "affineWitness": {"voxel": voxel_vertices[0].tolist(), "gltfM": mesh.vertices[0].tolist()},
    }
...
def validate_embedded_glb(payload: bytes, expected_names: set[str]) -> dict:
    ...
    if len(payload) < 20 or struct.unpack_from("<III", payload) != (0x46546C67, 2, len(payload)):
    ...
    def inspect(value):
        if isinstance(value, dict):
            ...
            for entry in value:
                inspect(entry)
    inspect(document)
    if any(document.get(key) for key in ["extensionsUsed", "extensionsRequired", "animations", "skins"]):
...
def convert(input_path: Path, labelmap_path: Path, license_path: Path, output: Path,
            fullnames_path: Path | None = None, units_reference_path: Path | None = None) -> dict:
    ...
    if "non-commercial" not in license_text or "permission" not in license_text:
        raise ValueError("Expected the local TopBrain non-commercial/permission license; review the supplied source.")
    ...
    if not int(image.header["sform_code"]) and not int(image.header["qform_code"]):
        raise ValueError("NIfTI must define an explicit sform or qform; fallback geometry is not accepted.")
    ...
    if source_unit == "unknown" and units_reference_path:
        ...
        if reference.shape != image.shape or not np.allclose(reference.affine, image.affine, rtol=0, atol=1e-6):
            raise ValueError("Units reference must have the identical voxel grid and affine as the label image.")
        resolved_unit = reference.header.get_xyzt_units()[0]
        units_evidence = {"method": "matching image header; identical shape and affine verified", "referenceSha256": checksum(units_reference_path)}
    transform = voxel_to_gltf_transform(image.affine, resolved_unit)
    volume = np.asanyarray(image.dataobj)
    if not np.isfinite(volume).all() or np.any(volume < 0) or np.any(volume != np.floor(volume)):
    ...
    if any(value not in labels for value in present):
        raise ValueError("At least one source label has no mapping; supply the matching release label map.")
    scene = trimesh.Scene()
    structures = []
    for value in present:
        ...
        mesh.visual = trimesh.visual.TextureVisuals(material=trimesh.visual.material.PBRMaterial(
            name=node_name, baseColorFactor=rgba, metallicFactor=0.0, roughnessFactor=0.65,
            doubleSided=True,
        ))
        scene.add_geometry(mesh, node_name=node_name, geom_name=node_name)
        short_name = entry["shortName"]
        side = "Right" if short_name.startswith("R-") else "Left" if short_name.startswith("L-") else "Not side-specific"
        group = "Arteries" if value <= 34 else "Veins and sinuses"
        explode = [0.04 if side == "Right" else -0.04 if side == "Left" else 0, 0.02, 0]
        structures.append({
            "id": node_name, "name": entry["name"], "label": value, "group": group,
            "side": side, "description": f"TopBrain batch-1 CTA label {value}: {short_name}. Name and color follow the supplied label map. {DISCLAIMER}",
            "color": color, "meshName": node_name, "explode": explode,
            "sourceShortName": short_name, "geometry": geometry,
        })
    ...
    dependencies = {name: importlib.metadata.version(name) for name in ["numpy", "nibabel", "scikit-image", "trimesh"]}
    manifest = {
        "schemaVersion": 1, "title": "TopBrain · real CTA vessel anatomy",
        ...
        "metadata": {
            "modality": "CTA", "release": "TopBrain batch 1, 2025-07-30",
            "sourceSha256": checksum(input_path), "labelmapSha256": checksum(labelmap_path),
            "fullnamesSha256": checksum(fullnames_path) if fullnames_path else None,
            "licenseSha256": checksum(license_path), "licenseText": license_text,
            "attribution": {"owner": OWNER, "title": SOURCE, "datasetUrl": WEBSITE,
                            "citationRequestedByLocalReadme": "https://arxiv.org/abs/2312.17670"},
            "sourceUnits": source_unit, "resolvedSourceUnits": resolved_unit, "unitsEvidence": units_evidence,
            "sourceShapeVoxels": list(image.shape),
            "sourceAffine": image.affine.tolist(), "sourceAffineSpace": "NIfTI RAS+ in resolvedSourceUnits",
            "sourceAxisCodes": list(nib.aff2axcodes(image.affine)),
            "sourceSpacing": nib.affines.voxel_sizes(image.affine).tolist(),
            "sformCode": int(image.header["sform_code"]), "qformCode": int(image.header["qform_code"]),
            "sourceUnitToMm": UNIT_TO_MM[resolved_unit],
            "rasMmToGltfM": RAS_MM_TO_GLTF_M.tolist(), "voxelToGltfM": transform.tolist(),
            "coordinateRule": "voxel -> full NIfTI affine -> RAS mm -> (x,z,-y)/1000 glTF meters; no centering baked into mesh",
            "processing": {"method": "marching_cubes", "level": 0.5, "gradientDirection": "ascent", "crop": "per-label voxel bounding box", "paddingVoxels": 1, "stepSize": 1, "smoothing": False, "decimation": False},
            "dependencies": dependencies, "python": platform.python_version(), "platform": platform.system(),
            "elapsedSeconds": round(time.perf_counter() - started, 3),
            "glbBytes": len(payload), "glbSha256": hashlib.sha256(payload).hexdigest(),
            "boundsGltfM": scene.bounds.tolist(), "clinicalUse": DISCLAIMER,
...
def main() -> None:
    ...
    parser.add_argument("--input", type=Path, required=True, help="Local 3-D CTA NIfTI label image")
    parser.add_argument("--labelmap", type=Path, required=True, help="Matching ITK-SNAP label map")
    parser.add_argument("--fullnames", type=Path, help="Optional label-name expansions supplied with release")
    parser.add_argument("--license", type=Path, required=True, help="Local release License.txt")
    parser.add_argument("--units-reference", type=Path, help="Matching image NIfTI header used only if label units are unknown; grid and affine must match")
    parser.add_argument("--output", type=Path, required=True, help="Output stem under ignored private-assets")
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    try:
        manifest = convert(args.input, args.labelmap, args.license, args.output, args.fullnames, args.units_reference)
        ...
        LOGGER.error("Conversion failed (%s). Check input format, spatial units, label map and destination permissions.", type(error).__name__)
        raise SystemExit(1) from None
    LOGGER.info("Converted %d labels; GLB %d bytes; %.3f seconds. %s", len(manifest["structures"]), manifest["metadata"]["glbBytes"], manifest["metadata"]["elapsedSeconds"], DISCLAIMER)

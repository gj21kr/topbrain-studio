...
export default function App() {
  ...
  const reset = (manifest: Manifest = asset.manifest) => { const defaults = initialDisplay(manifest.structures); setHidden(defaults.hidden); setOpacities(defaults.opacities); setSelected(defaults.selected); setIsolate(false); setExplosion(0); setOpacity(.85); setLabels(false); setAutoRotate(false); setView(v => ({ name: 'anterior', tick: v.tick + 1 })); };
  const replace = (next: Asset) => { disposeAsset(asset); setAsset(next); setQuery(''); setGroup('All structures'); reset(next.manifest); setError(''); };
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      <aside className="explorer"><div className="eyebrow">THE ANATOMY COLLECTION</div><h1>Head &amp; neck<span>Anatomy atlas</span></h1><p className="intro">구조를 선택하고, 공간 관계를<br/>직접 살펴보세요.</p><div className="dataset"><span className="dataset-icon"><Layers3 size={18}/></span><div><strong>{asset.scene ? 'Local anatomy study' : 'Interactive schematic'}</strong><small>{asset.scene ? 'Local geometry · source attached' : 'Illustrative sample · not TopBrain data'}</small></div></div>
      ...
      <main className="studio"><div className="scene-heading"><div><span className="eyebrow">{asset.scene ? 'LOCAL DATASET' : 'SPATIAL EXPLORATION'}</span><h2>{asset.manifest.title}</h2></div><span className="sample-badge">{asset.scene ? 'Imported geometry' : 'Schematic preview'}</span></div>
        <Viewer asset={asset} selected={selected} hidden={hidden} isolate={isolate} explosion={explosion} opacity={opacity} opacities={opacities} labels={labels} autoRotate={autoRotate} view={view} onSelect={select} onError={setError}/>
